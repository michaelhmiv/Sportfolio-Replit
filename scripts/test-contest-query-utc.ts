...
// Force UTC timezone (simulate Railway server)
process.env.TZ = 'UTC';
...
async function testContestQueryUTC() {
    let output = '';
    ...
    log('=== Testing Contest Query (UTC Timezone) ===\n');
    log(`Current TZ: ${process.env.TZ}`);
    ...
    fs.writeFileSync('contest_query_utc_test.txt', output, 'utf8');
    log('\nWritten to contest_query_utc_test.txt');
